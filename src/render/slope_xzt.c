...
/*   Updated: 2020/10/16 15:06:17 by anystrom         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	slope_dda_xzt(t_doom* dm, int side)
{
	...
	if (dm->pos.x + (dm->rayd.x * dm->walldist) - (int)dm->map.x <= 1 - (dm->pos.z + (dm->rayd.z * dm->walldist) - (int)dm->map.z))
		dm->hit = 1;
	if (dm->hit != 1)
	{
		...
		if ((dm->rmap2.z < 1 - dm->rmap2.x || dm->rmap2.z > LIM || dm->rmap2.x >= LIM)
			&& dm->rmap2.z > LIMN && dm->rmap2.x > LIMN)
		{
			dm->stepz = 1;
...
void	slope_dda_xztr(t_doom* dm, int side)
{
	...
	if (dm->hit != 1)
	{
		...
		if (dm->x == dm->winw / 2 && dm->y == dm->winh / 2)
			printf("XZTR %.16f < %.16f\n", dm->rmap2.z, dm->rmap2.y);
		if ((dm->rmap2.z > 1 - dm->rmap2.x || dm->rmap2.z < LIMN || dm->rmap2.x <= LIMN)
			&& dm->rmap2.z < LIM && dm->rmap2.x < LIM)
		{
			dm->stepz = 1;
			dm->sided.z += dm->deltad.z;
			dm->map.z += dm->stepz * create_plane_xzt(dm->rayd, dm->rmap1, dm, side);
